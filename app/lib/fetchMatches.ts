...
async function fetchMatches() {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 403) {
          console.warn(
            `Access to ${type} for ${leagueName} is restricted. Likely not available in free tier.`
          );
          return null;
        }
  ...
  if (
    matchesData.every(
      (data) =>
        data.recentMatches.length === 0 && data.upcomingFixtures.length === 0
    )
  ) {
    throw new Error(
      "No match data retrieved. Check API token or subscription plan."
    );
  }
