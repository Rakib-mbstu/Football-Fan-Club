...
async function fetchTop5Leagues() {
  ...
  if (!API_TOKEN) {
    throw new Error(
      "API token is missing. Please set FOOTBALL_DATA_API_TOKEN in .env.local"
    );
  }
  ...
  async function fetchLeagueStandings(
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/${leagueId}/standings?season=2025`,
        {
      ...
      if (!response.ok) {
        if (response.status === 403) {
          console.warn(
            `Access to ${leagueName} (${leagueId}) is restricted. Likely not available in free tier.`
          );
          return null;
        }
      ...
      console.error(
        `Error fetching standings for ${leagueName} (${leagueId}):`,
        error.message
      );
      return null;
    }
  ...
  if (standingsData.length === 0) {
    throw new Error(
      "No data retrieved for any league. Check API token or subscription plan."
    );
  }
